...
Game.launch = function () {
  ...
  Game.pickUpItem = function (iLvl) {
    ...
    if (Game.state.player.pickaxe.stats) {
      if (Game.state.player.pickaxe.stats.length > 0) {
        Game.state.player.pickaxe.stats.forEach(function (stat) {
          str += '<p>' + stat.name + ': ' + stat.val + '</p>';
        });
      }
    }
  ...
  Game.buildStore = function () {
    ...
    for (var _i3 in Game.buildings) {
      ...
      var price = _item.basePrice * (Math.pow(1.15, _item.owned + Game.state.prefs.buyAmount) - Math.pow(1.15, _item.owned)) / .15;
      if (_item.hidden == 0) {
